...
// Shared river-name normalizer — single source of truth so name-matching (recommendation
// text, restriction results, review fixes) can't drift between call sites.
function nrmName(s){return String(s||"").toLowerCase().replace(/[^a-z0-9]/g,"");}

// Deterministic reconciliation for "Best Bet Today". The recommendation text is written
// by the model in the same pass as the initial river list — BEFORE the closure check
// (labVerifyRestrictions) or the day-trip-range flag (labGovernor's outOfRange) are known.
// Those land later and only ever get patched onto the individual river cards, never back
// onto the recommendation text, so a closed or too-far river could keep sitting in
// "Best Bet Today" with nothing on screen saying so. Call this once both flags are settled
// (after restrictions resolve). If the recommended river is now ineligible, swap the
// recommendation to the next eligible river using a template — not a second AI call, same
// "wrong AI output gets a deterministic fix, not a better prompt" policy used elsewhere.
// Fail-open: if the recommended river can't be confidently identified, or every river is
// ineligible, never leave a blank/broken recommendation.
function reasonRiverIneligible(r){
  if(r.restriction){
    return r.restriction.status==="closure"?"closed to fishing right now":"under a hoot-owl restriction right now";
  }
  if(r.outOfRange){
    const hrs=r.driveMin?Math.round(r.driveMin/6)/10:null;
    return "more than a day trip away"+(hrs?" (~"+hrs+" h drive)":"");
  }
  return null;
}
function riverBlurb(r){
  const bits=[];
  const lead=(r.why||r.conditions||"").trim();
  if(lead) bits.push(/[.!?]$/.test(lead)?lead:lead+".");
  if(r.cfs&&r.cfs!=="unknown") bits.push("Currently "+r.cfs+" CFS"+(r.condition?" ("+r.condition+")":"")+".");
  if(r.driveMin!=null) bits.push("About "+r.driveMin+" min from your location.");
  if(r.bestTime) bits.push("Best fished "+r.bestTime+".");
  return bits.filter(Boolean).join(" ");
}
function coreRiverName(name){return String(name||"").replace(/\([^)]*\)/g,"").trim();}
function reconcileBestBet(report){
  if(!report||!report.recommendation||!Array.isArray(report.rivers)||!report.rivers.length) return report;
  const recNrm=nrmName(report.recommendation);
  // Match on the core name (parenthetical reach/corridor detail stripped) — prose often
  // paraphrases that part ("Kremmling to Dotsero corridor" vs the field's "Kremmling–Dotsero").
  // Longest name match wins, so "Colorado River" can't shadow "Upper Colorado River".
  let matched=null;
  for(const r of report.rivers){
    const rn=nrmName(coreRiverName(r.name));
    if(rn&&rn.length>=4&&recNrm.includes(rn)){
      if(!matched||rn.length>nrmName(coreRiverName(matched.name)).length) matched=r;
    }
  }
  if(!matched) return report; // can't confidently identify the recommended river — leave as-is
  const reason=reasonRiverIneligible(matched);
  if(!reason) return report; // recommended river is fine, nothing to reconcile

  const replacement=report.rivers.find(r=>r!==matched&&!reasonRiverIneligible(r));
  const swapDebug={originalPick:matched.name,reason,replacedWith:replacement?replacement.name:null};
  if(!replacement){
    return {...report,
      recommendation:matched.name+" is normally the top pick, but it's "+reason+". Every option nearby is currently closed or beyond day-trip range — check the river cards below for the closest workable option.",
      bestBetSwap:swapDebug};
  }
  const lead=matched.name+" is normally the strongest pick, but it's "+reason+" — so today's best bet is "+replacement.name+".";
  return {...report,
    recommendation:(lead+" "+riverBlurb(replacement)).trim(),
    bestBetSwap:swapDebug};
}
...

      {
            ...
            if(review){
              ...
              if(Array.isArray(fx.rivers)&&fx.rivers.length&&Array.isArray(nb.rivers)){
                nb.rivers=nb.rivers.map(rv=>{
                  const m=fx.rivers.find(f=>{const a=nrmName(f.name),b=nrmName(rv.name);return a&&b&&(a===b||a.startsWith(b)||b.startsWith(a));});
        ...
        if(restrictionsPromise&&builtReport){
          try{
            ...
            nb2=reconcileBestBet(nb2); // swap Best Bet Today off a river that just turned out closed / beyond day-trip range
