    ...
    try{
...

      {
        ...
        try{
          ...
          const reportTxt=await askClaude(`Fly fishing guide for ${loc.label}. Date: ${ds}. Weather: ${wxData?Math.round(wxData.current?.temperature_2m||0)+"°F, "+WX_DESC[wxData.current?.weather_code]:"unknown"}. Max drive: ${driveMinutes<60?driveMinutes+" min":Math.round(driveMinutes/60*10)/10+" hr"}. ${shopCtx}. List EVERY fishable stream within the drive time ranked best to worst. You MUST include Cache la Poudre, Big Thompson, North St Vrain, South St Vrain, Boulder Creek, South Boulder Creek, Clear Creek, South Platte (Deckers, Cheesman, Elevenmile), Arkansas River if reachable. Do not leave any out. Respond ONLY with JSON starting with {: {"overview":"...","recommendation":"...","rivers":[{"name":"...","lat":39.9,"lng":-105.1,"cfs":"...","condition":"Low/Normal/High/Optimal","conditions":"...","techniques":"...","flies":["Pattern #size"],"why":"..."}],"hatches":"...","bestTimes":"...","tips":"...","flyBoxEssentials":["Pattern #size"],"shops":${JSON.stringify(foundShops.map(s=>({name:s.name,website:s.website,reportUrl:s.reportUrl})))}}`,true,5000);
